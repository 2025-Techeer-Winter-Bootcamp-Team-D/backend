        ...
        try:
            ...
            for item in items:
                ...
                if any(k in nm for k in rev_keys) or aid in ['ifrs-full_Revenue', 'ifrs-full_OperatingRevenue']:
                    if is_finance:
                        ...
                        dart['rev'] = max(dart['rev'], val)

                ni_keys = ['순이익', '순손익', '분기순이익', '반기순이익']
                if any(k in nm for k in ni_keys) and '차감전' not in nm and '비지배' not in nm:
                ...
                if any(k in nm for k in ['매출원가', '영업원가']):
                    dart['cogs'] = val
                if any(k in nm for k in ['판매비관리비', '일반관리비']):
                    dart['sg_a'] = val
                if '영업비용' in nm:
                    dart['ope'] = val
            ...
            def align_value_final(val, reference, is_ni=False):
                ...
                if margin < 0.0001:
                    ...
                    if 0.001 < abs(boosted / reference) < 0.6:
                        return boosted
                if margin > 1000:
